...
inv_chain_dir = ngspice_dir / "inv_ring"
...
def plot_tb_moslv_inv_ring(show: bool=False):
    ...
    for i, num_inv in enumerate(num_inv_list):
        ...
        filepath_sp_gener = ref_dir_sp / f"tb_moslv_inv_ring_N{num_inv}_tt_generic.sp.out"
        filepath_sp_param = ref_dir_sp / f"tb_moslv_inv_ring_N{num_inv}_tt_tailored.sp.out"
        ...
        ax_i.plot(t_arr_sp_param[idx_arr_param], vout_arr_param[idx_arr_param], ls="-", color="blue", lw=3.0, label=f"vout, N={num_inv}")
        ax_i.plot(t_arr_sp_gener[idx_arr_gener], vout_arr_gener[idx_arr_gener], ls="--", color="orange", lw=2.0, label=f"vout, N={num_inv}")


    plt.savefig(fig_dir / (f"tb_moslv_inv_ring_tt" + ".png"), dpi=300)
