...
from dirs import ngspice_dir

inv_chain_dir = ngspice_dir / "inv_chain"
ref_dir_sp = inv_chain_dir / "ref"
...
def plot_tb_moslv_inv_chain_pulse(show=True, rfmode=0):

    # let's think about this plot
    # we want to plot the pulsed input on the top
    # and then plut the output for different N below that?
    ...
    for i, N in enumerate(N_list):
        ...
        if rfmode == 0:
            ...
            test_name = f"tb_moslv_rf_inv_chain_N{N}_tt"

        filepath_sp_gener = ref_dir_sp / (f"{test_name}" + "_generic" + '.sp.out')
        # filepath_sp_param = ref_dir_sp / (f"{test_name}" + "_paramset" + '.sp.out')
        filepath_sp_tailo = ref_dir_sp / (f"{test_name}" + "_tailored" + '.sp.out')

        data_sp1 = pd.read_csv(filepath_sp_gener, sep=r'\s+').values
        # data_sp2 = pd.read_csv(filepath_sp_gener, sep=r'\s+').values
        data_sp3 = pd.read_csv(filepath_sp_tailo, sep=r'\s+').values

        t_arr_sp1 = data_sp1[:, 0] / 1e-9
        ...
        t_arr_sp3 = data_sp3[:, 0] / 1e-9
        vout_arr_sp3 = data_sp3[:, 2]

        color = cmap(norm(N))

        ax1.plot(t_arr_sp1, vout_arr_sp1, ls="-", color=color, lw = 2.0, label=f"vout, N={N}")

        # ax0.plot(t_arr_sp2, vout_arr_sp2, ls="--", color='k', lw = 1.0)
        ax1.plot(t_arr_sp3, vout_arr_sp3, ls=":", color='r', lw = 1.0)
    ...
    plt.savefig(fig_dir / (f"tb_moslv_inv_chain_tt_pulsed" + ".png"), dpi=300)
...
def plot_tb_moslv_inv_chain(show=False):
    ...
    for i, N in enumerate(N_list):
        ...
        ax0.plot(t_arr_sp2, vout_arr_sp2, ls="--", color='k', lw = 1.0)
        ax0.plot(t_arr_sp3, vout_arr_sp3, ls=":", color='r', lw = 1.0)
    ...
    plt.savefig(fig_dir / (f"tb_moslv_inv_chain_tt" + ".png"), dpi=300)
