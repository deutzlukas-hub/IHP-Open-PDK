/*$Id: measure_average.cc,v 26.137 2010/04/10 02:37:05 al Exp $ -*- C++ -*-
...
public:
  std::string eval(CS& Cmd, const PARAM_LIST* Scope)const override{
    ...
    if (w) {
      ...
      return to_string(variance);
    }else{ untested();
      throw Exception_No_Match(probe_name);
    }
  }
} p4;
DISPATCHER<FUNCTION>::INSTALL d4(&measure_dispatcher, "sample_variance|svar", &p4);
/*--------------------------------------------------------------------------*/
}
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
