...
def parse_ac_data_gc(data_gc, mc_trials, num_ac_samples):

    C1_arr_gc = np.zeros(mc_trials)
    C2_arr_gc = np.zeros(mc_trials)

    start = 0
    for i in range(mc_trials):
        end = start + num_ac_samples
        data_i = data_gc[start:end, ...]
        freq_gc = data_i[:, 0]
        v_out_gc_1 = data_i[:, 1]
        v_out_gc_2 = data_i[:, 2]
        f_cutoff_gc_1, C1_gc = calculate_cap_from_f_cutoff(freq_gc, v_out_gc_1)
        f_cutoff_gc_2, C2_gc = calculate_cap_from_f_cutoff(freq_gc, v_out_gc_2)
        C1_arr_gc[i] = C1_gc
        C2_arr_gc[i] = C2_gc
        start = end

    return C1_arr_gc, C2_arr_gc
...
def plot_tb_cap_cutoff_mc_stat(plot_sp = True, show=False):
    ...
    # load gnucap data
    filepath_gc = ref_dir_gc / (test_name + ".gc.out")
    data_gc = pd.read_csv(filepath_gc, sep=r'\s+', header = None, names = ["#Freq", "v(out1)", "v(out2)"], skiprows=2, comment = '#', engine='python').values
    C1_arr_gc, C2_arr_gc = parse_ac_data_gc(data_gc, mc_trials, num_ac_samples)
